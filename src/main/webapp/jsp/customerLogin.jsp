    ...
    <div class="content">
        ...
        <div class="loader-container mt-4">
            <div class="loader"></div>
                ...
                <%
                boolean accessoEffettuato = CustomerDao.login(c);
                if (accessoEffettuato) {
                    // Azzera il contatore di tentativi falliti se il login ha successo
                    c.setFailedLoginAttempts(0);
                    CustomerDao.updateStatusEnabled(c);
                    session.setAttribute("loggedCustomer", CustomerDao.getRecordByUsername(c.getUsername()));
                    System.out.println("id customer prova dopo login " + c.getId());
                    %>
                    <script>
                    ...
                    <% 
                } else if (CustomerDao.getRecordByUsername(c.getUsername()).getStatus().equals("D")) {  
                    %>
                    <script>
                    ...
                    <%    
                } else if (CustomerDao.getRecordByUsername(c.getUsername()).getStatus().equals("A") && c.getFailedLoginAttempts() >= 3) { %>
                    <%  CustomerDao.updateStatusDisabled(c); %>
                    <script>
